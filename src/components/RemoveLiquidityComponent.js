...
export async function getPositionAndBalance(
  ...
  for (let pair of pairs) {
    ...
    userNonZeroLiquidityPositions.push([
      ...
      `pool balance: ${userPoolBalance.toExact()}/${
        userPoolBalance.token.symbol
      }`,
      `token0Balance: ${token0Deposited.toSignificant(6)} ${
        pair.token0.symbol
      }`,
      `token1Balance: ${token1Deposited.toSignificant(6)} ${
        pair.token1.symbol
      }`,
